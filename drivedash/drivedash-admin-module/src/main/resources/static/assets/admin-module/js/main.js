...
(function ($) {
  ...
  function aside() {
    if ($(window).width() > 1199) {
      ...
      $(".aside-toggle").on("click", function () {
        ...
        localStorage.setItem("isAsideFolded", body.hasClass('aside-folded') ? 'aside-folded': '');
  ...
  function asideFoldedShowSubMenu() {
    $('body.aside-folded .aside-body .main-nav > .has-sub-item').on('mouseenter', function(e) {
      let item = this.getBoundingClientRect();
      $(this).find('> .sub-menu').css('inset-block-start', (item.y - 60));
    });
  }
  ...
  $(document).ready(function() {
    $('.upload-file__input2').on('change',function(event){
        ...
        for (let i = 0; i < files.length; i++) {
          let file = files[i];
          $("<div class='file__value'><div class='file__value--text'>" + file.name + "</div><div class='file__value--remove' data-id='" + file.name + "' ></div></div>").insertBefore('#file__input');
      }
    });
  ...
  $(".circle-progress").each(function () {
    if ($(this).attr("data-parsent")) {
      $(this).css("background", function () {
        return (
          "conic-gradient(" +
          ($(this).attr("data-color") ? $(this).data("color") : "#14B19E") +
          " " +
          $(this).data("parsent") +
          "%, transparent 0)"
        );
  ...
  $('.form-control_color').on('change', function(e) {
    ...
    $(".search-submit").on('click', function () {
        setFilter($(this).data('url'), document.getElementById('search').value, 'search')
    })
    $(".date-range-change").on('change', function () {
        setFilter($(this).data('url'), document.getElementById('dateRange').value, 'date_range')
    })
